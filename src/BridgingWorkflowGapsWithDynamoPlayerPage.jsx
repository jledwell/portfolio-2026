...
import { bridgingWorkflowGapsWithDynamoPlayerContentData } from './bridgingWorkflowGapsWithDynamoPlayerContentData'
...
function BridgingWorkflowGapsWithDynamoPlayerPage() {
  useEffect(() => {
    document.title = 'Bridging Workflow Gaps with Dynamo Player'
  }, [])

  const contentBlocks = []
  for (let i = 0; i < bridgingWorkflowGapsWithDynamoPlayerContentData.length; i++) {
    const section = bridgingWorkflowGapsWithDynamoPlayerContentData[i]
    const next = bridgingWorkflowGapsWithDynamoPlayerContentData[i + 1]
    if (
  ...
  return (
    <div className="content-page content-page--dynamo-player">
      <Link to="/" className="content-back-link">
        ← Back to Portfolio
      </Link>
      <h1 className="content-page-title">
        Bridging Workflow Gaps with Dynamo Player
      </h1>
      <hr className="content-page-title-divider" aria-hidden="true" />
      {/* Tags hidden; data kept in bridgingWorkflowGapsWithDynamoPlayerContentTags */}
      {/* Summary grid hidden; data kept in bridgingWorkflowGapsWithDynamoPlayerSummaryColumns */}
      <div className="content-column">
        {contentBlocks.map((block, index) => {
          ...
          const isFull =
            ...
            section.heading === 'Customer Journey Map' ||
...
export default BridgingWorkflowGapsWithDynamoPlayerPage
