...
(() => {
  ...
  function load() {
    ...
    loadP = fetch(STATE_FILE)
      ...
      .catch(() => null)
      .then((j) => {
        // Standalone-bundle fallback: when the sidecar isn't fetchable
        // (file:// or single-file export), read state embedded in the page.
        if (!j || typeof j !== 'object') {
          const el = document.getElementById('__image_slots_embed');
          if (el) { try { j = JSON.parse(el.textContent); } catch (e) { j = null; } }
        }
